# -*- coding: utf-8 -*-
"""
Pruebas para el endpoint GET /representative/students/<id>/attendance/

Cubre los casos obligatorios (feliz, inválido, sin autenticar, repetición),
más la prueba con mock.patch para el caso 403 (permiso denegado por parentesco).
"""
...
class RepresentativeStudentAttendanceTests(APITestCase):

    def setUp(self):
        self.usuario = User.objects.create_user(username='representante_att', password='clave-de-prueba')
        self.student_id = 4412
        self.periodo_id = 6
        self.url = reverse('movil-representative-student-attendance', kwargs={'student_id': self.student_id})

    def test_obtiene_asistencia_estudiante_representado_exitoso(self):
        """Camino feliz: devuelve 200 OK con la estructura de asistencia."""
        ...
        self.assertIn('subjects', r.data['data'])
    ...
    @patch('sige_ports.portal.asistencia_de_estudiante', side_effect=LookupError('No hay asistencia'))
    def test_asistencia_no_encontrada_devuelve_404(self, mock_asistencia):
